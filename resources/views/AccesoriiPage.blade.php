@extends("loyaut")
@section("title", "$accesorii->name_acces")
@section("content")
<div class="oferte-div">
    <h2 class="despre-txt">Despre accesoriu: {{$accesorii->name_acces}}</h2>
</div>
<div class="despre-bicicleta">
  <img src="{{ asset($accesorii->image) }}" alt="BikesImage">

  <div class="bike-details">
    <h3>{{ $accesorii->name_acces }}</h3>
    <h3>Accesoriu de:{{$accesorii->type_acces}}</h3>
    <h4 class="price">Prețul: {{ $accesorii->price }} MDL</h4>
    <h4 class="color">Culoarea: {{ $accesorii->color_acces }}</h4>
    <h4 class="stock">Este în stoc</h4>

    <form action="{{ route('cart.add') }}" method="POST">
        @csrf
        <input type="hidden" name="accesorii_id" value="{{ $accesorii->id }}">
